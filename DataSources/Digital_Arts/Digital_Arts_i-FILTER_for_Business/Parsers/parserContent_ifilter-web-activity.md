...
Name = ifilter-web-activity
    ...
    Fields = [
      ...
      """\w+ \d{1,100} \d\d:\d\d:\d\d \d{1,100} \S+ (-|({dest_ip}[A-Fa-f:\d.]+)) (-|({src_ip}[A-Fa-f:\d.]+)) (-|({src_host}\S+)) (-|({user}[^\s]+)) \[({time}\d{1,100}\/\w+\/\d\d\d\d:\d\d:\d\d:\d\d [+-]\d{1,100})\] ({result_code}\d{1,100}) ({bytes_out}\d{1,100}) ({bytes_in}\d{1,100}) ({action}\S+) \S+ ({reason}\S+) ({category}\S+).+?"({method}\S+) ({full_url}(({protocol}\w+):[\\\/]+)?({web_domain}[^\\\/\s:]+)({uri_path}\/[^\s\?]+)?({uri_query}\?[^\s"]+)?)[^"]*" (-|({referrer}\S+)) (-|({user_agent}.+?)) \S+ \S+\s{0,100}$""",
