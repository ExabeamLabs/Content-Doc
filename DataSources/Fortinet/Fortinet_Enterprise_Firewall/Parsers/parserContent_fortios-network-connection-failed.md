...
Name = fortios-network-connection-failed
  ...
  Fields = [
    ...
    """\smsg="{0,20}({additional_info}[^\s"]*)"{0,20}""",
    """\slogdesc="{0,20}({event_name}[^\s"]*)"{0,20}""",
    """\stranport=({src_translated_port}\d{1,100})""",
    """\stranip=({dest_translated_ip}[a-fA-F\d.:]+)""",
    """\ssrcname="{0,20}({src_host}[^\s"]*)"{0,20}"""
  ]
}
```