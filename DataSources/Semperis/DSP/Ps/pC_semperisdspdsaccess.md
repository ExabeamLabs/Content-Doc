...
Name = semperis-dsp-ds-access
  ...
  """OriginatingUsers\]\s(NT AUTHORITY|({domain}[^\;]{1,2000}))[\\]{1,100}(SYSTEM|({user}[^;]{1,2000}))"""
  """OriginatingUserWorkstations\]\s({src_host}[\w\-.]{1,2000})"""
  """ClassName\]\s({object_class}[^\s]{1,2000})"""
  """DistinguishedName\]\s({object_dn}[^\[]{1,2000}?)\s{1,20}\["""
  """AttributeName\]\s({attribute}[^\s]{1,2000})"""
  """StringValueFrom\]\s({old_attribute}[^\s]{1,2000})"""
