...
Name = proofpoint-email-3
    ...
    Conditions = [ """routeDirection""", """requestMethod=ProofPointMessageLog_CL""" ]
    Fields = [
      ...
      """"filter_verified_rcpts_s":"\[\s*\\+"+({recipient}[^,]+?@({external_domain_recipient}[^,]+?))\\+"+"""
      ...
      """"filter_isMsgReinjected_b":\s*({is_consolidated}\w+),"""
      """"rule":\s*"({rule_name}[^"]+)""""
    ]
