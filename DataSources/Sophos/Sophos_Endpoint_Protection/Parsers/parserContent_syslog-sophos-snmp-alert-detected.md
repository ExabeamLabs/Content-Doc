...
Name = syslog-sophos-snmp-alert-detected
  ...
  Fields = [
    ...
    """:=\s{0,100}({additional_info}[^"]+?)\s{1,100}"({file_path}(({file_parent}.+)[\\\/])?({file_name}.+?))"\s{1,100}of\s{1,100}controlled application\s{1,100}'({alert_name}.+?)'\s{0,100}\(of type\s{1,100}({alert_type}.+?)\)\s{1,100}has been detected\.""",
  ]
