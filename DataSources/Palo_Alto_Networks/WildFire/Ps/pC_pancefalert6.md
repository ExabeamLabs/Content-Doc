...
```Java
{
Name = pan-cef-alert-6
  DataType = "network-alert"
  TimeFormat = "MMM dd yyyy HH:mm:ss zzz"
  Conditions = [ """Palo Alto Networks|PAN-OS|""","""Microsoft Windows user enumeration(30842)|THREAT|""" ]
...
pan-cef-alert = {
  ...
  Fields = [ """exabeam_EventTime=({eventtime}\d{1,100})""",
    ...
    """duser=(({domain}[^\\="]{1,2000})[\\\/]{1,20})?({user}[^\s@"]{1,2000})(@({=domain}[^\s@=]{1,2000}))?\s\w+=""",
    """suser=(({domain}[^\\=]{1,2000})\\+)?({user}[^\s@"]{1,2000})(@({=domain}[^\s@=]{1,2000}))?\s\w+=""",
