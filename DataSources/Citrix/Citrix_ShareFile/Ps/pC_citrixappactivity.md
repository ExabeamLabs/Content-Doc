...
Name = citrix-app-activity
  ...
  Conditions = ["""destinationServiceName =Citrix ShareFile""", """"Activity":""", """"Email":"""]
  Fields = ${CitrixParserTemplates.citrix-app-activity.Fields}[
    """"ItemName"{1,20}:"{1,20}({file_path}({file_parent}[^"]{0,2000}?[\/]{1,20})?({file_name}[^\/"]{1,2000}?(\.({file_ext}[^\/"\s\.]{1,2000}))?))""""
  ]
