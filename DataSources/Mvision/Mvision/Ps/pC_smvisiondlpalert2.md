...
```Java
{
Name = s-mvision-dlp-alert-2
  DataType = "dlp-alert"
  Conditions = [ """"threattype":""",""""OUTGOING_EMAIL"""", """"PolicyName":""" ,""""threatseverity":""" ]
  Fields = ${MvisionParserTemplates.s-mvision-dlp-alert.Fields}[
    """"EmailSubject":\s{0,100}"({subject}[^"]{1,2000})"""",
    """"MatchedEmailRecipients":\s{0,100}"({recipients}[^"]{1,2000}?)\s{0,100}"""",
    """"EmailSender":\s{0,100}"({sender}[^@"]{1,2000}@[^\.]{1,2000}\.[^"]{1,2000})""""
  ]
}
s-mvision-dlp-alert = {
    Vendor = Mvision
