...
Name = json-5145
    ...
    Fields = [
      ...
      """ObjectType"{1,20}:"{1,20}({file_type}[^"]+)""",
      """ShareName"{1,20}:"{1,20}[\\\*]*({share_name}[^"]+)""",
      """ShareLocalPath"{1,20}:"{1,20}(?:[\\\?]+)?(|({share_path}(({d_parent}.+?)\\\\)?(|({d_name}[^\\]*?)))\\?)"""",
      """RelativeTargetName"{1,20}:"{1,20}({f_parent}(?:[^"]+)?[\\\/])?({file_name}[^\\:"]+?(\.\s{0,100}({file_ext}[^"\\.]+?))?)"""", 
      """AccessList"{1,20}:"{1,20}({accesses}[^"]+)""",
