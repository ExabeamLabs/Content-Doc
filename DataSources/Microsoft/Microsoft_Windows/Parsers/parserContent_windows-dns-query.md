...
Name = windows-dns-query
  ...
  Fields = [
    ...
    """({protocol}UDP)\s{1,100}({activity}Rcv)\s{1,100}({src_ip}[a-fA-F\d.:]+)""",
    """Remote addr\s{0,100}({src_ip}[a-fA-F\d.:]+),\s{0,100}port\s{0,100}({src_port}\d{1,100})""",
    """XID\s{1,100}0x({query_id}[\da-fA-F]+)""",
    """QTYPE\s{1,100}({query_type}\w+)""",
    """RCODE\s{1,100}[^(]*?\(({result}[^(]+)\)""",
