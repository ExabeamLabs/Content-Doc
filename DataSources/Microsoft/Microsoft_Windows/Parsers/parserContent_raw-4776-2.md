...
Name = raw-4776-2
    ...
    Fields = [
      ...
      """Logon (?:a|A)ccount(:|=)\s{0,100}(({user_email}[^@\s]+?@[^\s]+?\.[^\s]+?)|(({user}[^@\s,;=]+?)(?:@({domain}[^\s.;,@=]+).*?)?)|({=user}.+?))[\s;]*Source Workstation(:|=)""",
      """Error Code(:|=)\s{0,100}({result_code}[\w\-]+)""",
      """Source Workstation(:|=)([\s\\]+|(\s{0,100}\\*(((::ffff:)?({dest_ip}(\d{1,3}\.){3}\d{1,3}|([A-Fa-f0-9%.]*:[A-Fa-f0-9%.:]+))(:({dest_port}\d{1,100}))?)|(::ffff:)?({dest_host}[^\s]+?))[\s;]*))Error Code(:|=)"""
    ]
  }
```