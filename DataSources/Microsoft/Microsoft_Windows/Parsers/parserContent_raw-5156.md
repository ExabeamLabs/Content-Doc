...
Name = raw-5156
  ...
  Fields = [
    """Microsoft-Windows-Security-Auditing.*?({time}\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d)\s{1,100}(::ffff:)?(am|AM|pm|PM|({host}[\w.\-]+))""",
    """(?i)\w+ \d{1,100} \d\d:\d\d:\d\d (::ffff:)?(am|pm|({host}[\w\-.]+))""",
    """\WComputer\\*=(::ffff:)?({host}[\w\-.]+)""",
    """\WComputerName:\s{0,100}(::ffff:)?({host}[\w\-.]+)""",
    """({time}\w+ \d\d \d\d:\d\d:\d\d \d\d\d\d)\s{1,100}""",
    """(?i)\w+\s{0,100}\d{1,100}\s{0,100}\d{1,100}:\d{1,100}:\d{1,100}\s(::ffff:)?(am|pm|({host}[\w\-.]+))""",
    """({event_code}5156)""",
