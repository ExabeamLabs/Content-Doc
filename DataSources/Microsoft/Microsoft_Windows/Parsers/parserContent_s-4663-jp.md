...
Name = s-4663-jp
  ...
  Fields = [
    ...
    """アカウント名:\s{0,100}({user}[^\s]+)""",
    """アカウント ドメイン:\s{0,100}({domain}[^\s]+)""",
    """ログオン ID:\s{0,100}({logon_id}[^\s]+)""",
    """オブジェクトの種類:\s{0,100}({file_type}[^\s]+)""",
    """オブジェクト名:\s{0,100}({file_path}.+?)\s{0,100}ハンドル ID:""",
    """オブジェクト名:\s{0,100}({file_parent}.+?)({file_name}([^\\:]+(?=\.))({file_ext}\.[^\\:]+?)?|[^\\:]+?)\s{0,100}ハンドル ID:""",
    """プロセス名:\s{0,100}({process}({directory}.+?[\\\/])?({process_name}[^\\\/"]+?))\s{0,100}アクセス要求情報:""",
