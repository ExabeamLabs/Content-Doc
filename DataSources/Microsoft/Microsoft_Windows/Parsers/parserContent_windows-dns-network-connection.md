...
Name = windows-dns-network-connection
  ...
  Fields = [
    ...
    """RemoteAddress=({dest_ip}[^\s]+)\s{1,100}""",
    """RemotePort=({dest_port}.+?)\s{0,100}ProcessName=""",
    """Direction=({direction}.+?)\s{0,100}Protocol=""",
    """RemoteHostName=({host}[^\s]+)\s{1,100}""",
    """ProcessName="{0,20}(unknown|(({process_name}({directory}([^\\]+\\)+)?({process}.+?))))"{0,20}\s{0,100}UserName=""",
    """Protocol=({protocol}.+?)\s{0,100}ProtocolId=""",
    """LocalAddress=({src_ip}[^\s]+)\s{1,100}""",
    """LocalPort=({src_port}.+?)\s{0,100}RemoteHostName=""",
    """UserSid=(unknown|({user_sid}.+?))\s{0,100}UserId=""",
    """UserName="{0,20}(unknown|((nt authority|({domain}[^\\\/]+))[\\\/])?([Ss]ystem|localsystem|network service|({user}.+?)))"{0,20}\s{0,100}UserSid=""",
