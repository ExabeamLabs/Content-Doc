...
Name = raw-failed-logon-2003
    ...
    Fields = [
      ...
      """(?i)(((audit|failure)( |_)(audit|failure))|information)\s{0,100}(\s|\t|,|#\d{1,100}|<[^>]+>)\s{0,100}({host}[^=]+?)\s{0,100}(\s|\t|,|#\d{1,100}|<[^>]+>)\s{0,100}""",
      """\d\d:\d\d:\d\d \d\d\d\d\s{0,100}(\s|\t|,|#\d{1,100}|<[^>]+>)\s{0,100}({event_code}\d{1,100})\s{0,100}(\s|\t|,|#\d{1,100}|<[^>]+>)\s{0,100}Security""",
      ...
      """Workstation Name:\s{0,100}(-|({src_host_windows}[^\s]+))\s{0,100}Caller User Name:""",
      """Workstation Name:\s{0,100}({src_host}[^\s]+)\s{0,100}Caller User Name:.*?Source Network Address:\s{0,100}-\s{1,100}""",
      """Caller User Name:\s{0,100}(?:-|({caller_user}.+?))\s{0,100}Caller Domain:""",
      """Caller Domain:\s{0,100}({caller_domain}.+?)\s{0,100}Caller Logon ID:\s{0,100}\([^,]+,({logon_id}[^\)]+)""",
      """Source Network Address:\s{0,100}({src_ip}[a-fA-F:\d.]+)"""
    ]
