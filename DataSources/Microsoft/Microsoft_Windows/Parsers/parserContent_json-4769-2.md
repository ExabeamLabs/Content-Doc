...
Name = json-4769-2
  ...
  Fields = ${WinParserTemplates.json-windows-events-2.Fields}[
    ...
    """ServiceName\\?"{1,20}:\\?"{1,20}({dest_host}[^\s\\]+)""",
    """IpAddress\\?"{1,20}:\\?"{1,20}(::[\w]+:)?({src_ip}[\da-fA-F.:]+)\\?"""",
    """Status\\?"{1,20}:\\?"{1,20}({result_code}[^\s\\]+)""",
    """TicketOptions\\?"{1,20}:\\?"{1,20}({ticket_options}[^\s\\]+)""",
    """TicketEncryptionType\\?"{1,20}:\\?"{1,20}({ticket_encryption_type}[^\s\\]+)"""
 ]
