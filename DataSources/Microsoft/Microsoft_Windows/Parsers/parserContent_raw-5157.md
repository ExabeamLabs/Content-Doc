...
Name = raw-5157
  ...
  Fields = [
    ...
    """\sProcess ID:\s{0,100}(|({process_id}\d{1,100}))\s{0,100}Application Name:\s{0,100}(|({process}({directory}.*?[\\\/]+)?({process_name}[^\\\/]+?)))\s{0,100}Network Information:""",
    """\sDirection:\s{0,100}({direction}\S+)""",
    """\sSource Address:\s{0,100}({src_ip}[a-fA-F\d.:]+)""",
    """\sSource Port:\s{0,100}({src_port}\d{1,100})""",
    """\sDestination Address:\s{0,100}({src_ip}[a-fA-F\d.:]+)""",
