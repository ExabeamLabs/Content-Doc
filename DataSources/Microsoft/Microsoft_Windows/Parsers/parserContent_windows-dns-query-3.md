...
Name = windows-dns-query-3
  ...
  Fields = [
    ...
    """"({time}\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d.\d{0,20}Z)",([^\s]{1,2000}\s){3}PACKET\s{1,100}\S{1,2000}\s{1,100}({protocol}\S{1,2000})\s{1,100}({activity}\S{1,2000})\s{1,100}({src_ip}[a-fA-F\d.:]{1,2000})\s{1,100}\S{1,2000}\s{1,100}Q\s{1,100}\[\S{1,2000}\s{1,100}(\s|({query_flags}[^\s]{1,2000}))\s{1,100}\S{1,2000}\]\s{1,100}({query_type}\S{1,2000})\s{1,100}({query}[^",]{1,2000})","""
  ]
}
```