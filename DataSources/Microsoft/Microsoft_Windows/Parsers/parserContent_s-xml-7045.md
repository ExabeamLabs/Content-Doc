...
Name = s-xml-7045
  ...
  Fields = [
    ...
    """<Data Name='ServiceName'>({service_name}[^<]+?)\s{0,100}</Data>""",
    """<Data Name='ServiceType'>({service_type}[^<]+?)\s{0,100}</Data>""",
    """<Data Name='ImagePath'>({process}({directory}[^<>"]*?[\\\/]+)?({process_name}[^"<>\\\/]*))</Data>""",
    """<Data Name='ImagePath'>({command_line}"({process}({directory}[^<>"]*?[\\\/]+)?({process_name}[^"<>\\\/]*))"(\s{0,100}({arg}[^<>]+))?)</Data>""",
