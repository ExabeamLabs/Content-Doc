...
Name = cef-snare-5140
  ...
  Fields = [
    ...
    """({accesses}Read)""",
    """\Wad\.ShareLocalPath=(?:[\\\?]+)?(?:\s{0,100}|({share_path}({d_parent}.*?)({d_name}[^\\]+?))(\\+)?)(\s{1,100}(\w+|\w+\.\w+)=|\s{0,100}$)""",
    """\WfileType=({file_type}\w+)""",
    """\WcategoryOutcome=\/?({outcome}.+?)\s{1,100}(\w+=|$)""",
    """\WObject Type:\s{0,100}({file_type}.+?)\s{1,100}Source Address:\s{0,100}({src_ip}[A-Fa-f:\d.]+)""",
    """\WShare Name:\s{0,100}({share_name}.+?)\s{1,100}Share Path:""",
    """\WShare Path:\s{0,100}(|({share_path}({d_parent}.*?[\\\/]+)?(|({d_name}[^\\]+?))))\s{1,100}Access Request Information:""",
  ]
}
```