...
Name = u-mcafee-epo-alert
    ...
    Fields = [
      ...
      """<TargetFileName>({malware_url}[^=]*?[\\\/]*\s{0,100}({malware_file_name}[^\s\\\/<][^\\\/<]*?))\\?<""",
      """<AnalyzerDetectionMethod>({additional_info}[^<]+)""",
      """<OSName>({os}[^<]+)""",
      """<ThreatActionTaken>(none|([^\._<]+[\.|_]){0,256}({outcome}[^<]+))<""",
      """<AnalyzerName>(N\/A|({event_name}[^<]+))""",
      """<TaskName>({task_name}[^<]+)""",
      """<TargetPath>({process}({process_directory}[^<]*?)\s{0,100}(({process_name}[^\s<\\\/][^<\\\/]*?)?))<""",
      """<TargetName>\s{0,100}({process_name}[^\s<][^<]*?)<""",
      """<SourceProcessName>({src_process_name}[^<]+)<""",
      """<TargetHash>\s{0,100}({md5}[^\s<][^<]*)<""",
      """<Cleanable>({cleanable}[^<]+)<""",
    ]
    SOAR {
