...
Name = websense-proxy-2
  ...
  Fields = [
    ...
    """\Wcache="{0,20}({proxy_action}.+?)"{0,20},?\s{0,100}(\w+=|$)""",
    """\WReferer="{0,20}({referrer}.+?)"{0,20},?\s{0,100}(\w+=|$)""",
    """\Whttp_content_type="?(?:-|({mime}.+?))"?,?\s{0,100}(\w+=|$)""",
    """\Whttp_user_agent="{0,20}({user_agent}[^"]{1,2000}?)(\s"|")""",
    """\Whttp_user_agent="{0,20}(?:-|Mozilla\/.+\([^\)]{0,2000}?({os}iOS|Android|BlackBerry|Windows Phone|BeOS|(?:X|x)11|(?:W|w)indows|(?:L|l)inux|(?:M|m)acintosh|(?:D|d)arwin).+?({browser}Chrome|Safari|Opera|(?:F|f)irefox|MSIE|Trident))""",
    """\Wuser="?({user}.+?)"?,?\s{0,100}(\w+=|$)""",
    """\Wuser="?LDAP.+?DC\\*=org\/({user}.+?)"?,?\s{0,100}(\w+=|$)"""
  ]
}
```