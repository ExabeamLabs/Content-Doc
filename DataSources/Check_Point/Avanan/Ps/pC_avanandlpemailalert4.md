...
Name = avanan-dlp-email-alert-4
  ...
  Fields = ${AvananParserTemplates.json-avanan-security-alert.Fields}[
    """"description_text\\{0,10}":\\{0,10}"[^"']{1,2000}?'({subject}[^"']{1,2000})'""",
    """"saas_info\\{0,10}":\[\{[^\}]{1,2000}?"full_name\\{0,10}":\\{0,10}"({user_fullname}[^\\"]{1,2000})\\{0,10}"""",
    """"sender_address":"({sender}[^@"]{1,2000}@[^\."]{1,2000}\.[^"]{1,2000})"""",
    """"saas_info":[^\}]{1,2000}"email":"({user_email}[^@"]{1,2000}@[^\."]{1,2000}\.[^"]{1,2000})"""",
    """"is_deleted\\{0,10}":({outcome}[^,]{1,200})"""
  ]
  DupFields = [ "user_email->recipient" ]
