...
```Java
{
Name = gcp-disksinsert-json
  DataType = "gcp-disk-create"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """googleapis.com""", """"methodName":""", """compute.disks.insert"""" ]
  Fields = ${GcpParserTemplates.gcp-cloudaudit-json.Fields}[
    """"source({source_resource_type}Image)":\s{0,100}"({source_resource}[^"\\]*)"""",
    """"source({source_resource_type}Snapshot)":\s{0,100}"({source_resource}[^"\\]*)"""",
  ]
