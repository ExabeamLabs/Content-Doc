...
```Java
{
Name = gcp-updaterole-json
  DataType = "gcp-role-write"
  ...
  Fields = ${GcpParserTemplates.gcp-cloudaudit-json.Fields}[
    """"{1,20}addedPermissions"{1,20}:\s{0,100}\[({added_permissions}[^\\\/\]]{1,2000})\]""",
    """"{1,20}removedPermissions"{1,20}:\s{0,100}\[({removed_permissions}[^\\\/\]]{1,2000})\]""",
    """"{1,20}response"{1,20}:[^\]\[]+"{1,20}included_permissions"{1,20}:\s{0,100}\[({role_permissions}[^\\\/\]]{1,2000})\]""",
  ]
