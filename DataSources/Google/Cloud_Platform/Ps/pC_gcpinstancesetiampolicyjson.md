...
```Java
{
Name = gcp-instancesetiampolicy-json
  DataType = "gcp-policy-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """googleapis.com""", """"methodName":""", """compute.instances.setIamPolicy"""" ] 
  Fields = ${GcpParserTemplates.gcp-cloudaudit-json.Fields}[
    """"{1,20}policy"{1,20}:[^\}]{1,1000}"{1,20}bindings"{1,20}:\s{0,100}\[\s{0,100}({policy_bindings}.+)\s{0,100}\],?[\s\]\
gcp-cloudaudit-json = {
    Vendor = Google
