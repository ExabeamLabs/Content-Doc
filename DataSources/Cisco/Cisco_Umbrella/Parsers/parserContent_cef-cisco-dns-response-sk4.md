...
Name = cef-cisco-dns-response-sk4
  ...
  Fields = [
    ...
    """"identities":\["({user_fullname}[^\("]+?)(?:\s{0,100}\(\w+\)\s{0,100})?(\s{1,100}\(({user_email}[^@"]+@[^@"]+)\))",("({host}[\w\-\.]+)")?""",
    """"mostGranularIdentity":"({user_fullname}[^\("]+?)(?:\s{0,100}\(\w+\)\s{0,100})?(\s{1,100}\((({user_email}[^@"]+@[^@"]+)|({user}[^"]+))\))"""",
    ...
    """"domain":"({query}[^"]*?)\.?"""",
    """"categories":\s{0,100}\[({categories}"{0,20}({category}[^"\],]+)[^\]]*)\]""",
    """"externalIp"{1,20}:"{1,20}({dest_ip}[^"]+)"""
  ]
}
```