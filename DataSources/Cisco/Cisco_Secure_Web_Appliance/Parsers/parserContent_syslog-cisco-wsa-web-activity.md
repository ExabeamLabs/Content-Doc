...
Name = syslog-cisco-wsa-web-activity
  ...
  Fields = [
    """exabeam_host=({host}[^\s]+)""",
    """accesslog_syslog:\s\S+\s({time}\d{10})\.\d{3}\s\S+\s({src_ip}[\d.:a-fA-F]+)\s((-|(?i)NONE|({proxy_action}[^\s\/]+?))(\/(-|({result_code}\d{1,100})))?)\s\d{1,100}\s(-|({method}[^\s]+))""",
    """accesslog_syslog:(\s\S+){7}\s(-|({full_url}(({protocol}[^:]+):\/+)?[^\s:\/]+(:({dest_port}\d{1,100}))?\/(?:-|({uri_path}[^?\s]+))?({uri_query}\?[^\s]+)?))""",
    """accesslog_syslog:(\s\S+){8}\s"{0,20}(-|(({domain}[^\\]+)\\+)?({user}[^@"\s]+))""",
    ...
    """accesslog_syslog:(\s\S+){12}\s<(["-]+|nc|({category}[^,>]+?))\s{0,100}[,>]""",
