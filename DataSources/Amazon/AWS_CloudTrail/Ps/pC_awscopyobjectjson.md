...
```Java
{
Name = aws-copyobject-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-storageobject-copy"
  ...
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    ...
    """"{1,20}requestParameters.+?copy-source\\?":\s{0,100}\\?"({src_file_path}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?key\\?":\s{0,100}\\?"({file_path}[^"]{1,2000}?)\\?"""",
    """"{1,20}additionalEventData.+?bytesTransferredIn\\{0,20}":\s{0,100}({bytes_in}\d{1,1000})""",
    """"{1,20}additionalEventData.+?bytesTransferredOut\\{0,20}":\s{0,100}({bytes_out}\d{1,1000})""",
    """"{1,20}resources.+?Object.+?(?:ARN|arn)\\?":\s{0,100}\\?"({src_file_arn}[^"]{1,2000}?)\\?"""",
    """"{1,20}resources.+?Bucket.+?(?:ARN|arn)\\?":\s{0,100}\\?"({src_bucket_arn}[^"]{1,2000}?)\\?"""",
    """"{1,20}resources.+?Object.+?Object.+?(?:ARN|arn)\\?":\s{0,100}\\?"({dest_file_arn}[^"]{1,2000}?)\\?"""",
    """"{1,20}resources.+?Bucket.+?Bucket.+?(?:ARN|arn)\\?":\s{0,100}\\?"({dest_bucket_arn}[^"]{1,2000}?)\\?"""",
  ]
