...
```Java
{
Name = aws-createpolicyversion-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-policy-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreatePolicyVersion"""" ]
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
  """"{1,20}requestParameters":\{("[^,]+,){0,10}"policyArn\\?":\s{0,100}\\?"({policy_arn}[^"]{1,2000}?\/({policy}[^"]{1,2000}))\\?"""",
  """"{1,20}requestParameters":\{("[^,]+,){0,10}"policyArn\\?":\s{0,100}\\?"({policy_arn}[^"]{1,2000}?)\\?"""",
  """"{1,20}policyDocument"{1,20}:\s{0,10}"{1,20}\{[^\[]+({policy_content}"{1,20}Statement\\?"{1,20}.+\])\s{0,100}(?:\\n)?\s{0,100}\}"{1,20}""",
  """"{1,20}Effect\\?":\s{0,100}\\?"Allow\\?".+?Action\\?":\s{0,100}\[?({allowed_permissions}[^\.\]\}]+)\]?""",
  """"{1,20}Effect\\?":\s{0,100}\\?"Allow\\?".+?Resource\\?":\s{0,100}\[?({allowed_resources}[^\.\]\}]+)\]?""",
  """"{1,20}Effect\\?":\s{0,100}\\?"Deny\\?".+?Action\\?":\s{0,100}\[?({denied_permissions}[^\.\]\}]+)\]?""",
  """"{1,20}Effect\\?":\s{0,100}\\?"Deny\\?".+?Resource\\?":\s{0,100}\[?({denied_resources}[^\.\]\}]+)\]?""",
  """"{1,20}requestParameters":\{("[^,]+,){0,10}"setAsDefault\\?":\s{0,100}\\?"?({set_as_defualt}[^"\}\s]{1,2000})\\?""",
  """"{1,20}responseElements":\{"policy":\{("[^,]+,){0,10}"versionId\\?":\s{0,100}\\?"({policy_version_id}[^"]{1,2000}?)\\?"""",
  ]
