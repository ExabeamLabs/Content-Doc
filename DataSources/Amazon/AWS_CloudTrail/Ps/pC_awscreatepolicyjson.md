...
```Java
{
Name = aws-createpolicy-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-policy-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreatePolicy"""" ]
  ...
  """"{1,20}Effect\\?":\s{0,100}\\?"Allow\\?".+?Action\\?":\s{0,100}\[?({allowed_permissions}[^\.\]\}]+)\]?.{0,10}\s{0,100}"{1,20}Resource?""",
  """"{1,20}Effect\\?":\s{0,100}\\?"Allow\\?".+?Resource\\?":\s{0,100}\[?({allowed_resources}[^\.\]\}]+)\]?""",
  """"{1,20}Effect\\?":\s{0,100}\\?"Deny\\?".+?Action\\?":\s{0,100}\[?({denied_permissions}[^\.\]\}]+)\]?.{0,10}\s{0,100}"{1,20}Resource?""",
  """"{1,20}Effect\\?":\s{0,100}\\?"Deny\\?".+?Resource\\?":\s{0,100}\[?({denied_resources}[^\.\]\}]+)\]?""",
  """"{1,20}responseElements":\{"policy":\{("[^,]+,){0,10}"policyName\\?":\s{0,100}\\?"({policy}[^"]{1,2000}?)\\?"""",
  """"{1,20}responseElements":\{"policy":\{("[^,]+,){0,10}"policyId\\?":\s{0,100}\\?"({policy_id}[^"]{1,2000}?)\\?"""",
  """"{1,20}responseElements":\{"policy":\{("[^,]+,){0,10}"arn\\?":\s{0,100}\\?"({policy_arn}[^"]{1,2000}?)\\?"""",
  ]
