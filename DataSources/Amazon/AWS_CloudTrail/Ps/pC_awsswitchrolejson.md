...
```Java
{
Name = aws-switchrole-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-role-switch"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsConsoleSignIn""", """"eventName":"SwitchRole"""" ]
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
  """"{1,20}additionalEventData":\{("[^,]+,){0,10}"RedirectTo\\?"{1,20}:\s{0,100}\\?"({full_url}[^"]{1,2000}\/({role}[^"]{1,2000}))\\?"{1,20}""",
  """"{1,20}additionalEventData":\{("[^,]+,){0,10}"SwitchTo\\?":\s{0,100}\\?"({role_arn}[^"]{1,2000}?)\\?"""",
  """"{1,20}additionalEventData":\{("[^,]+,){0,10}"SwitchFrom\\?":\s{0,100}\\?"({source_role}[^"]{1,2000}?)\\?"""",
  ]
