...
```Java
{
Name = aws-modifyimageattribute-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-image-modify"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"ModifyImageAttribute"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?imageId\\?":\s{0,100}\\?"({resource_id}[^"]{1,2000}?)\\?"""",
    """"{1,20}launchPermission.+?add.+items\\?":\s{0,100}\\?\[({added_users}[^\]]{1,2000}?)\\?\]""",
    """"{1,20}launchPermission.+?remove.+items\\?":\s{0,100}\\?\[({removed_users}[^\]]{1,2000}?)\\?\]""",
  ]
