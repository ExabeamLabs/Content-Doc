...
```Java
{
Name = aws-addusertogroup-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-identity-addtogroup"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"AddUserToGroup"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?groupName\\?":\s{0,100}\\?"({group_name}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?userName\\?":\s{0,100}\\?"({identity}[^"]{1,2000}?)\\?"""",
  ]
