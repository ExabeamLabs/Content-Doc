...
```Java
{
Name = aws-createimage-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-image-create"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreateImage"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?({source_resource_type}[Ss]napshot|[Ii]nstance)Id\\?":\s{0,100}\\?"({source_resource}[^",]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?name\\?":\s{0,100}\\?"({image_name}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?[Dd]escription\\?":\s{0,100}\\?"({description}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?imageId\\?":\s{0,100}\\?"({resource_id}[^"]{1,2000}?)\\?"""",
  ]
