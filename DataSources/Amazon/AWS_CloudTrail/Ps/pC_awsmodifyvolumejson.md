...
```Java
{
Name = aws-modifyvolume-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-volume-modify"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"ModifyVolume"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?volumeId\\?":\s{0,100}\\?"({resource_id}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?volumeType\\?":\s{0,100}\\?"({volume_type}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?originalMultiAttachEnabled\\?":\s{0,100}\\?({old_multiattach}[^",]{1,2000}?)\\?,""",
    """"{1,20}responseElements.+?targetMultiAttachEnabled\\?":\s{0,100}\\?({new_multiattach}[^",]{1,2000}?)\\?,""",
    """"{1,20}responseElements.+?originalSize\\?":\s{0,100}\\?({old_size}[^",]{1,2000}?)\\?,""",
    """"{1,20}responseElements.+?targetSize\\?":\s{0,100}\\?({new_size}[^",]{1,2000}?)\\?,""",
  ]
