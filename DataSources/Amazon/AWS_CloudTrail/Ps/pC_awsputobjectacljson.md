...
```Java
{
Name = aws-putobjectacl-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-storage-acl"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"PutObjectAcl"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    ...
    """"{1,20}AccessControlList"{1,20}:\s{0,10}\{[^\[]*({acl_content}"{1,20}Grant"{1,20}.+\])(?:\\n)?\s{0,100}\
aws-cloudtrail-json = {
    Vendor = Amazon
    Product = AWS CloudTrail
    Lms = Direct
