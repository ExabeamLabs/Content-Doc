...
```Java
{
Name = aws-modifyinstanceattribute-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-instance-modify"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"ModifyInstanceAttribute"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?instanceId\\?":\s{0,100}\\?"({resource_id}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?userData\\?":\s{0,100}\\?"({userdata}[^"]{1,2000}?)\\?"""",
  ]
