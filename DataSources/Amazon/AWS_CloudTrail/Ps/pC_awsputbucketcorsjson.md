...
```Java
{
Name = aws-putbucketcors-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-bucket-cors"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"PutBucketCors"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    ...
    """"{1,20}AllowedMethod\\?":\s{0,100}\{?\[?({allowed_permissions}[^\]\}]+)\]?\}?.+?"AllowedOrigin"""",
    """"{1,20}AllowedOrigin\\?":\s{0,100}\{?\[?({allowed_origins}[^\]\,\s}]+)\]?\}?""",
    """"{1,20}AllowedHeader\\?":\s{0,100}\{?\[?({allowed_headers}[^\]\,\s}]+)\]?\}?""",
