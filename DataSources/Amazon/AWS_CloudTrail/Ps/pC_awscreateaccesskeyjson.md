...
```Java
{
Name = aws-createaccesskey-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-identity-creds-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreateAccessKey"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}responseElements.+?userName\\?":\s{0,100}\\?"({identity}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?accessKeyId\\?":\s{0,100}\\?"({key_id}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?status\\?":\s{0,100}\\?"({key_status}[^"]{1,2000}?)\\?"""",
  ]
