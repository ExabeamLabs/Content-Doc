...
```Java
{
Name = aws-createvolume-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-volume-create"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreateVolume"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?size\\?":\s{0,100}\\?({volume_size}[^",]{1,2000}?)\\?,""",
    """"{1,20}requestParameters.+?zone\\?":\s{0,100}\\?"({volume_zone}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?volumeType\\?":\s{0,100}\\?"({volume_type}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?snapshotId\\?":\s{0,100}\\?"({source_resource}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?volumeId\\?":\s{0,100}\\?"({resource_id}[^"]{1,2000}?)\\?"""",
  ]
