...
```Java
{
Name = aws-consolelogin-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-login"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsConsoleSignIn""", """"eventName":"ConsoleLogin"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"responseElements\\?"\s{0,100}:\s{0,100}.+?\s{0,100}\\?".+?\\?"\s{0,100}:\s{0,100}\\?"({status}[^"]{1,2000}?)\\?"""",
    """"{1,20}LoginTo"{1,20}\s{0,100}:\s{0,100}"{1,20}?({full_url}[^"].+?)"{1,20}\s{0,100}[,\]\}]""",
    """"{1,20}MobileVersion"{1,20}\s{0,100}:\s{0,100}"{1,20}?({mobile_version}[^"].+?)"{1,20}\s{0,100}[,\]\}]""",
    """"{1,20}MFAUsed"{1,20}\s{0,100}:\s{0,100}"{1,20}?({mfa}[^"].+?)"{1,20}\s{0,100}[,\]\}]""",
  ]
