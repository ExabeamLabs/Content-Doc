...
```Java
{
Name = aws-updatefunctioncode-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-function-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"UpdateFunctionCode""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?functionName\\?":\s{0,100}\\?"({function_arn}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?functionName\\?":\s{0,100}\\?"({function}[^"]{1,2000}?)\\?"""",
