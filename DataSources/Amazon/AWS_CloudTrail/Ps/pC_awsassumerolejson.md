...
```Java
{
Name = aws-assumerole-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-role-assume"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"AssumeRole"""" ]
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
  """"{1,20}requestParameters[^\}]*?roleArn\\?"{1,20}:\s{0,100}\\?"({role_arn}[^"]{1,2000}\/({role}[^"]{1,2000}))\\?"{1,20}""",
  """"{1,20}requestParameters":\{("[^,]+,){0,10}"roleSessionName\\?":\s{0,100}\\?"({session_name}[^"]{1,2000}?)\\?"""",
  """"{1,20}responseElements":\{"credentials":\{("[^,]+,){0,10}"expiration\\?":\s{0,100}\\?"({session_expiration}[^"]{1,2000}?)\\?"""", 
  """"assumedRoleUser":\{("[^,]+,){0,10}"arn"\s{0,100}:\s{0,100}"({session_arn}[^"]{1,2000})\\?""""
  ]
