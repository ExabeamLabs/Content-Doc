...
```Java
{
Name = aws-modifysnapshotattribute-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-snapshot-modify"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"ModifySnapshotAttribute"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?snapshotId\\?":\s{0,100}\\?"({resource_id}[^"]{1,2000}?)\\?"""",
    """"{1,20}createVolumePermission.+?add.+items\\?":\s{0,100}\\?\[({added_users}[^\]]{1,2000}?)\\?\]""",
    """"{1,20}createVolumePermission.+?remove.+items\\?":\s{0,100}\\?\[({removed_users}[^\]]{1,2000}?)\\?\]""",
  ]
