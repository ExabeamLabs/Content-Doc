...
```Java
{
Name = aws-putgrouppolicy-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-policy-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"PutGroupPolicy"""" ]
  ...
  """"{1,20}requestParameters":\{("[^,]+,){0,10}"groupName\\?":\s{0,100}\\?"({identity}[^"]{1,2000}?)\\?"""",
  """"{1,20}requestParameters":\{("[^,]+,){0,10}"policyName\\?":\s{0,100}\\?"({policy}[^"]{1,2000}?)\\?"""",
  ]
