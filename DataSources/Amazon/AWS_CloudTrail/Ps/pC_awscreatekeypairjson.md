...
```Java
{
Name = aws-createkeypair-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-instance-creds-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreateKeyPair"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?keyName\\?":\s{0,100}\\?"({key_name}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?keyType\\?":\s{0,100}\\?"({key_type}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?keyPairId\\?":\s{0,100}\\?"({resource_id}[^"]{1,2000}?)\\?"""",
  ]
