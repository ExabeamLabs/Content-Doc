...
```Java
{
Name = aws-sendcommand-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-instance-command"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"SendCommand"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}targets.+?values\\?":\s{0,100}\\?\[({resource_id}[^\]\}]{1,2000}?)\\?\]""",
    """"{1,20}requestParameters.+?documentName\\?":\s{0,100}\\?"({document_name}[^\]\}"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?PlaybookFile\\?":\s{0,100}\\?\[({playbook_files}[^\]\}]{1,2000}?)\\?\]""",
  ]
