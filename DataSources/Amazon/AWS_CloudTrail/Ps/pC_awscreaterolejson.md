...
```Java
{
Name = aws-createrole-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-role-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreateRole"""" ]
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
  """"{1,20}assumeRolePolicyDocument\\?".+?"{1,20}Effect\\?":\s{0,100}\\?"Allow\\?".+?AssumeRole.+?Principal\\?":\s{0,100}\[?\{?({allowed_users}[^\]\}]+)\]?\}?""",
  """"{1,20}assumeRolePolicyDocument\\?".+?"{1,20}Effect\\?":\s{0,100}\\?"Deny\\?".+?AssumeRole.+?Principal\\?":\s{0,100}\[?\{?({denied_users}[^\]\}]+)\]?\}?""",
  """"{1,20}responseElements":\{"role":\{("[^,]+,){0,10}"roleName\\?":\s{0,100}\\?"({identity}[^"]{1,2000}?)\\?"""",
  """"{1,20}responseElements":\{"role":\{("[^,]+,){0,10}"roleId\\?":\s{0,100}\\?"({identity_id}[^"]{1,2000}?)\\?"""",
  """"{1,20}responseElements":\{"role":\{("[^,]+,){0,10}"arn\\?":\s{0,100}\\?"({identity_arn}[^"]{1,2000}?)\\?"""",
  ]
