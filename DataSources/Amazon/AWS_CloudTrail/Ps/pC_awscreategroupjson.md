...
```Java
{
Name = aws-creategroup-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-identity-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreateGroup"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}responseElements.+?groupName\\?":\s{0,100}\\?"({identity}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?groupId\\?":\s{0,100}\\?"({identity_id}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?arn\\?":\s{0,100}\\?"({identity_arn}[^"]{1,2000}?)\\?"""",
  ]
