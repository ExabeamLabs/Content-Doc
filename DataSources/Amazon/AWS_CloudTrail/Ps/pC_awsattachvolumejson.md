...
```Java
{
Name = aws-attachvolume-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-volume-attach"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"AttachVolume"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?volumeId\\?":\s{0,100}\\?"({resource_id}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?instanceId\\?":\s{0,100}\\?"({instance_id}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?device\\?":\s{0,100}\\?"({volume_device}[^"]{1,2000}?)\\?"""",
  ]
