...
```Java
{
Name = aws-createfunction-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-function-write"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreateFunction""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?functionName\\?":\s{0,100}\\?"({function}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?runtime\\?":\s{0,100}\\?"({function_runtime}[^"]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?role\\?":\s{0,100}\\?"({function_role}[^"]{1,2000}?)\\?"""",
