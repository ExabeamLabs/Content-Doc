...
```Java
{
Name = aws-updateassumerolepolicy-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-role-assumepolicy"
  ...
  """"{1,20}Effect\\?":\s{0,100}\\?"Allow\\?".+?Principal\\?":\s{0,100}\[?\{?({allowed_users}[^\]\}]+)\]?""",
  """"{1,20}Effect\\?":\s{0,100}\\?"Deny\\?".+?Principal\\?":\s{0,100}\[?\{?({denied_users}[^\]\}]+)\]?""",
  ]
