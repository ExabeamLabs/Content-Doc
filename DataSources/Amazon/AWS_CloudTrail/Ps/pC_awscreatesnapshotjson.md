...
```Java
{
Name = aws-createsnapshot-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-snapshot-create"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"CreateSnapshot"""" ] 
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
    """"{1,20}requestParameters.+?({source_resource_type}volume|[Ii]nstance)Id\\?":\s{0,100}\\?"({source_resource}[^",]{1,2000}?)\\?"""",
    """"{1,20}requestParameters.+?[Dd]escription\\?":\s{0,100}\\?"({description}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?snapshotId\\?":\s{0,100}\\?"({resource_id}[^"]{1,2000}?)\\?"""",
    """"{1,20}responseElements.+?ownerId\\?":\s{0,100}\\?"({owner_id}[^"]{1,2000}?)\\?"""",
  ]
