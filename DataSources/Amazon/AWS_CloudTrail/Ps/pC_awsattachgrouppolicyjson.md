...
```Java
{
Name = aws-attachgrouppolicy-json
  Vendor = Amazon
  Product = AWS CloudTrail
  Lms = Direct
  DataType = "aws-policy-attach"
  TimeFormat = """yyyy-MM-dd'T'HH:mm:ss.SSSSSSZ"""
  Conditions = [ """AwsApiCall""", """"eventName":"AttachGroupPolicy"""" ]
  Fields = ${AwsParserTemplates.aws-cloudtrail-json.Fields}[
  """"{1,20}requestParameters":\{("[^,]+,){0,10}"groupName\\?":\s{0,100}\\?"?({identity}[^"\}\s]{1,2000})\\?""",
  """"{1,20}requestParameters":\{("[^,]+,){0,10}"policyArn\\?":\s{0,100}\\?"?({policy_arn}[^"\}\s]{1,2000})\\?""",
  ]
  DupFields = ["policy_arn->policy"]
