...
Name = cef-okta-logs-app-activity
  ...
  Fields=[
    ...
    """request":.+?"displayName":"(Okta System|(?:({user_firstname}[^,"]+),\s*({user_lastname}[^"]+)|({user_fullname}[^"]+)))"""",
    """request":.+?"alternateId":"(system@okta\.com|(?:({user_email}[^"@]+@({domain}[^"]+))|({user}[^"]+)))"""",
    """"actor":\s*[^\]]*?"displayName"\s*:\s*"(Okta System|(?:({user_lastname}[^,"]+),\s*({user_firstname}[^"]+)|({user_fullname}[^"]+)))"""",
    """"actor":[^\]]*?"alternateId"\s*:\s*"(system@okta\.com|(?:({user_email}[^"@]+@({domain}[^"]+))|({user}[^"]+)))"""",
