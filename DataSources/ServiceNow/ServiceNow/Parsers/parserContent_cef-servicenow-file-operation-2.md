...
Name = cef-servicenow-file-operation-2
  ...
  Fields = [
    ...
    """\Wsuser=(anonymous|system|({user_email}[^\s@]+@({email_domain}[^\s@]+))|({user}[^=]+?))(\s{1,100}\w+=|\s{0,100}$)""",
    """"srcip":"({src_ip}[^"]+)""",
    """"name":"({object}[^"]+)",""",
    ...
    """CEF:([^\|]*\|){5}({activity}[^\|]+)""",
    """"queue":"({activity}[^"]+)""",
    """"parm1":"{1,20}\s{0,100}(|-|({resource}[^"]+?))\s{0,100}",""",
    """"(instance|documentkey)":"({object}[^"]+?)",""",
    """"tablename":"({table_name}[^"]+)",""",
    """"table":"({table}[^"]+)",""",
    """dproc=({event_name}[^=]+?)(\s\w+=|$)""",
    """msg=({additional_info}[^=]+?)(\s\w+=|$)""",
    """"parm2":"\s{0,100}({action}[^"]+?)\s{0,100}",""",
    """"file_name":"({file_name}[^"]+?(\.({file_ext}[^\."]+))?)",""",
    """"size_bytes":"({bytes}\d{1,100})""",
    """"content_type":"({file_type}[^"]+?)",""",
    """"oldvalue":"\s{0,100}({old_value}[^"]+?)\s{0,100}",""",
    """newvalue"{1,20}:"\s{0,100}({new_value}[^"]+?)\s{0,100}",""",
  ]
