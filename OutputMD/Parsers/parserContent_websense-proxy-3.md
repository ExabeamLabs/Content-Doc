...
Name = websense-proxy-3
    ...
    Fields = [
      ...
      """websense_wsg\|([^\^]+\^){3}(?:-|({action}[^\^]+))\^(?:-|({protocol}[^\^]+))\^(?:-|({result_code}[^\^]+))\^[^\^]+\^(?:-|[^\^]*?=(({domain}[^\^\\\/=]+)[\\\/]+)?({user}[^\^\\\/=]+))\^(?:-|({src_ip}[^\^]+))\^(?:-|({src_port}[^\^]+))\^(?:-|({dest_ip}[^\^]+))\^(?:-|({dest_port}[^\^]+))\^(?:-|({web_domain}[^\^]+))\^[^\^]+\^(?:-|({bytes_out}[^\^]+))\^(?:-|({bytes_in}[^\^]+))\^([^\^]+\^){9}(?:-|({method}[^\^]+))\^(?:-|({mime}[^\^]+))\^[^\^]+\^(?:-|({user_agent}[^\^]+))\^[^\^]+\^(?:-|({full_url}[^\^]+?))\s*(\^|$)""",
      ...
      """websense_wsg\|([^\^]+\^){30}(?:-|\w+:\/\/[^\/]+({uri_path}[^\^\?]*?)(\?({uri_query}[^\^]*?))?)\s*(\^|$)"""
    ]
  }
```