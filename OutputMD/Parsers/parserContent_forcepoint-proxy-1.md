...
Name = forcepoint-proxy-1
    ...
    Fields = [
      ...
      """user=.+?DC\\?=\w+/(({user_email}[^@=\s]+@[^@=]+?)|({user_fullname}[^@=]+?)(@[^@=]*?)?)(\s+\w+=|\s*$)""",
	    ...
	    """\surl=([^=]*?)({top_domain}(?!(?:\d+\.){3}\d+)[^\.\s\/:]+(?=(?:\.(?:com|net|info|edu|org|gov|co|jp|ru|de|ir|it|in|fr|info|pl|nl|es|gr|cz|eu|tv|me|jp|ca|cn|uk|my|cc|id|us|nz|biz|club|io|gg|fi|au|st|tw|asia|sg|ie|li|za|im))+(\s|\/|$))[^\s\/]+)""",
